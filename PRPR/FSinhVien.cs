...
namespace PRPR
{
    public partial class FSinhVien : Form
    {
        ...
        {
            string hoten = svdao.GetHoTenByMaSo(ms);
            linkLabel1.Text = ms + "\n" + hoten;
            ...
            try
            {
                dataGridView1.DataSource = svdao.ThongBao(ms);
        ...
        private void label3_Click(object sender, EventArgs e)
        {
            ...
            {
                currentFormChild.Close();
                FSinhVien_Load(sender as Form, e);
            }
        }
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            FInfor fInfor = new FInfor(ms);
            OpenchildForm(fInfor);
        }
        ...
        private void label5_Click(object sender, EventArgs e)
        {
            ...
            FSignUp fSignUp = new FSignUp(ms);
            OpenchildForm(fSignUp);
        }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0) // Kiểm tra xem người dùng đã nhấp vào một dòng hợp lệ hay không
            {
                DataGridViewRow row = dataGridView1.Rows[e.RowIndex];

                string value1 = row.Cells["matb"].Value.ToString();

                string noiDungThu = svdao.DocThongBao(value1);
        ...
        {
            DataTable dataTable = svdao.KTThongBao(ms);
            if (dataTable.Rows.Count > 0) // Nếu có kết quả từ truy vấn
            {
                DataRow row = dataTable.Rows[0];
                string maDeTai = row["madetai"].ToString();
                string fileName = "SEO-icon.png";
                string imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, fileName);
                Image image = Image.FromFile(imagePath);
                pictureBox10.Image = image;
                label10.Text = "BÁO CÁO";
                FReport fReport = new FReport(ms, maDeTai); // Truyền mã sinh viên và mã đề tài vào form báo cáo
                OpenchildForm(fReport);
            }
            else
            {
        ...
        private void label7_Click(object sender, EventArgs e)
        {
            ...
            FMarkSV fMarkSV = new FMarkSV(ms);
            OpenchildForm(fMarkSV);
        }
    }
}
